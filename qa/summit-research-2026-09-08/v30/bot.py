"""Summit v30: public range equity and correlated-evidence tempering.
...
def decide(s,memory=None):
    ...
    # Preserve a lead, but retain linear chip value when behind or tied.
    # Stack geometry, not opponent identity, determines risk sensitivity.
    ours=stack+get(s['committed'],seat)
    theirs=get(s['stacks'],1-seat)+get(s['committed'],1-seat)
    power=.4+.6*min(1.,theirs/max(1,ours))
