"""Summit v23: public range equity and correlated-evidence tempering.
...
def likelihood(q,price,action,kind):
    ...
    if kind==1:
        ...
        # Equity-based players use noisy samples. Model the probability
        # their estimate crosses a threshold, not just the true equity.
        noise=sqrt(max(.001,q*(1-q))/20.+.0025)
        raise_p=.025+.91*.5*erfc((.70-q)/(1.41421356237*noise))
        cont=.02+.97*.5*erfc((price+.16-q)/(1.41421356237*noise))
