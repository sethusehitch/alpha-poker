"""Summit v19: public range equity and correlated-evidence tempering.
...
def decide(s,memory=None):
    ...
    priors=[.75,max(.04,.20-.3*aggression),max(.04,.6*aggression-.08)]
    particles=[]
    for _ in range(96):
        o=rng.sample(deck,2)
    ...
    for target in targets:
        ...
        value=0.; reraises=0.; reraise_equity=0.
        # One extra response layer: an opponent can reraise, not merely
        # fold or call. Choose our response using the aggregate reraising
        # range, never separately for the sampled hidden holdings.
        bump=min(max(0,rem-extra),pot+tc+2*extra)
        for ws,oq,eq in particles:
            for kind,w in enumerate(ws):
                fold=likelihood(oq,price,'fold',kind)
                rr=likelihood(oq,price,'raise',kind) if bump>0 else 0.
                passive=max(0.,1-fold-rr)
                value+=w*(fold*pot+passive*((eq-premium)*(pot+tc+2*extra)-tc-extra))
                reraises+=w*rr
                reraise_equity+=w*rr*eq
        value+=max(-reraises*(tc+extra),
                   (reraise_equity-premium*reraises)*(pot+tc+2*extra+2*bump)-reraises*(tc+extra+bump))
