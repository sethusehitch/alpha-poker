"""Summit v31: public range equity and correlated-evidence tempering.
...
def decide(s,memory=None):
    ...
    # At short stacks few future opportunities remain, so return toward
    # linear chip value instead of paying a large risk premium indefinitely.
    bb=max([e.get('amount',0) for e in s['action_history'] if e.get('type') in ('small_blind','big_blind')] or [20])
    effective=min(stack+get(s['committed'],seat),get(s['stacks'],1-seat)+get(s['committed'],1-seat))
    power=.4+.6/(1.+effective/max(1,bb)/10.)
    utility=lambda chips:max(0.,chips)**power
    base=utility(stack)
