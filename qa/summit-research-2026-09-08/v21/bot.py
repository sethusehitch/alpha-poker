"""Summit v21: public range equity and correlated-evidence tempering.
...
def decide(s,memory=None):
    ...
    if not b:
        # Weak preflop hands realize less than their all-in showdown equity
        # when chips remain for multiple later betting rounds.
        suited=h[0]%4==h[1]%4
        premium=(.035+.13*max(0.,min(1.,(.55-q)/.25))-.015*suited)*min(1.,spr/2)
