    ...
    {!loaded ? <p role="status">Loading your profile…</p> : !session ? <section className="rounded-3xl border border-zinc-200 bg-white p-8 text-center">
      ...
      {current && <section className="mt-8 rounded-3xl border border-zinc-200 bg-white p-5 shadow-sm sm:p-8">
        {welcome || editing ? <>
          ...
          <div className="h-44 w-44"><CharacterImage username={session.username} avatar={current.avatar} alt="Your profile picture" /></div>
          <h2 className="mt-5 text-xl font-semibold">Profile picture</h2>
          <button onClick={() => { setEditing(true); setSaved(false); }} className="mt-6 rounded-xl bg-blue-600 px-6 py-3 font-semibold text-white hover:bg-blue-700">Change picture</button>
          {saved && <p role="status" className="mt-4 text-sm font-medium text-green-700">Profile picture saved.</p>}
        </div>}
      </section>}
