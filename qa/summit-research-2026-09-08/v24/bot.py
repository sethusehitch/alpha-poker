"""Summit v24: public range equity and correlated-evidence tempering.
...
def decide(s,memory=None):
    ...
    # Learn persistent style from each completed hand's predictive evidence.
    # Apply that evidence once, not once per decision in the same hand.
    priors=mem.get('model_prior',[.8,.1,.1])
    if mem.get('model_hand')!=s['hand_id']:
        evidence=mem.pop('model_evidence',None)
        if evidence:
            maximum=max(evidence)
            updated=[w*max(.01,e/maximum)**.5 for w,e in zip(priors,evidence)]
            z=sum(updated);priors=[.01+.97*w/z for w in updated]
        mem['model_prior']=priors
        mem['model_hand']=s['hand_id']
    particles=[]
    for _ in range(96):
        o=rng.sample(deck,2)
    ...
    mem['model_evidence']=[mass/max(1e-30,prior) for mass,prior in zip(masses,priors)]
