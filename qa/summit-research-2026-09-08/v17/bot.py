"""Summit v17: public range equity and correlated-evidence tempering.
...
def decide(s,memory=None):
    ...
    for _ in range(160):
        o=rng.sample(deck,2)
        qs={k:public_equity(o,b[:k],rng,24) for k in streets}
        ws=priors[:]
        ...
        for _ in range(8 if len(b)<5 else 1):
            board=b+rng.sample(rest,5-len(b));a,z=rank(h+board),rank(o+board)
            score+=(a>z)+.5*(a==z)
        eq=score/(8 if len(b)<5 else 1)
