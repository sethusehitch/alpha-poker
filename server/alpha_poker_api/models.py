...
class TrainingCreate(BaseModel):
    ...
    hand_limit: int = Field(default=400, ge=1, le=400)
    client_schema_version: Literal["2026-09-01"] = "2026-09-01"


class RunCreate(BaseModel):
    # Kept for backward compatibility with prototype operator scripts. The
    # tournament engine now decides hand count by bankruptcy, not a fixed cap.
    hand_count_per_pairing: int | None = Field(default=None, ge=1, le=100000)
    seed: int | None = None
